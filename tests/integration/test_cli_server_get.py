...
class TestCLIServerGet:
    ...
    def test_put_and_get_simple_credential(self, run_mattstash_cli, cli_env):
        ...
        # Put credential
        result = run_mattstash_cli(["put", "test-api-key", "--value", "secret-value-123"], cli_env)
        ...
        result = run_mattstash_cli(["get", "test-api-key", "--show-password"], cli_env)
        assert result.returncode == 0
    ...
    def test_get_with_json_output(self, run_mattstash_cli, cli_env):
        ...
        run_mattstash_cli(["put", "json-test", "--value", "json-value"], cli_env)

        # Get with JSON
        result = run_mattstash_cli(["get", "json-test", "--json", "--show-password"], cli_env)
        assert result.returncode == 0
        assert "{" in result.stdout
