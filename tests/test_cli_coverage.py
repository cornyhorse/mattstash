...
from mattstash.models.credential import Credential, serialize_credential
...
def test_main_put_value_mode(temp_db: Path):
    ...
    result = main(["--db", str(temp_db), "put", "test-key", "--value", "test-value"])
    assert result == 0
...
def test_main_put_value_mode_with_json(temp_db: Path):
    ...
    with patch("builtins.print") as mock_print:
        result = main(["--db", str(temp_db), "put", "test-key", "--value", "test-value", "--json"])
        assert result == 0
...
def test_main_put_fields_mode_with_json(temp_db: Path):
    ...
    with patch("builtins.print") as mock_print:
        result = main(["--db", str(temp_db), "put", "test-cred", "--fields", "--username", "testuser", "--json"])
        assert result == 0
        mock_print.assert_called()
...
def test_main_get_nonexistent(temp_db: Path):
    ...
    result = main(["--db", str(temp_db), "get", "nonexistent"])
...
def test_main_delete_nonexistent(temp_db: Path):
    ...
    result = main(["--db", str(temp_db), "delete", "nonexistent"])
    assert result == 2
...
def test_main_versions_empty(temp_db: Path):
    ...
    with redirect_stdout(output):
        result = main(["--db", str(temp_db), "versions", "nonexistent"])
...
def test_main_versions_with_json(temp_db: Path):
    ...
    with patch("builtins.print") as mock_print:
        result = main(["--db", str(temp_db), "versions", "test", "--json"])
        assert result == 0
...
def test_main_db_url_error(temp_db: Path):
    ...
    result = main(["--db", str(temp_db), "db-url", "nonexistent"])
...
def test_main_s3_test_missing_credential(temp_db: Path):
    ...
    result = main(["--db", str(temp_db), "s3-test", "nonexistent"])
