...
class DatabaseBootstrapper:
    ...
    def _create_database_and_sidecar(self, db_dir: str, sidecar_path: str) -> None:
        ...
        self._create_empty_database(pw, sidecar_path)

    def _create_empty_database(self, password: str, sidecar_path: str) -> None:
        """Create an empty KeePass database with the given password."""
        try:
            if _kp_create_database is None:
                raise RuntimeError("pykeepass.create_database not available in this version")
            _kp_create_database(self.db_path, password=password)
